...
#list[
    论文写作时排版软件一般不能直接引用数据库，而是引用某种_标准格式的导出文件_，最常用的格式是BiBLaTeX格式，扩展名为`.bib`。从Zotero中可以将数据库导出成该格式文件。
    #set par(first-line-indent: 0em)
    `.bib`文件中包含有很多_条目_，每个条目有一个*键值*，用于引用该条目。键值一般是一个短字符串，由Zotero自动生成，保证唯一性。常见的方式是_作者姓名加发表年份_。例如张三在2023年发表的论文，键值是`张三2023`。
]
