...
论文写作时排版软件一般不能直接引用数据库，而是引用某种_标准格式的导出文件_，最常用的格式是BiBLaTeX格式，扩展名为`.bib`。从Zotero中可以将数据库导出成该格式文件。`.bib`文件中包含有很多_条目_，本文`.bib`文件中的第一个条目内容如下：
...
每个条目有一个*键值*，用于引用该条目。键值一般是一个短字符串，由Zotero自动生成，保证唯一性。常见的方式是_作者姓名加发表年份_。例如张三在2023年发表的论文，键值是`张三2023`。

- 在Typst中排版论文时，用交叉引用相同的语法进行外部引用，只是_用键值代替标签_。具体方法见@交叉引用节。

== 手稿版本管理

学位论文写作是一个长期的过程，往往需要几个月甚至几年，在漫长的过程中_需要反复增删改_。这个过程中留下每次增删改的详细记录，随时追溯不同版本的变化非常重要。

最简单的方法是每天保存一个版本文件，按日期命名，但这种方法过于简陋。更好的方法是向程序员学习，使用版本管理软件。这类软件不少，但当前最流行的是Git，功能强大、支持广泛，建议采用。
#show: 正文结束
= 结论
...
#bibliography("references/reference.bib", style: "gb-7714-2005-numeric")
